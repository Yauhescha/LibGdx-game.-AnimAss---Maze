...
public class MazeGenerator {
    ...
    public static String[][] generate(int size) {
        ...
        emptyHash(mazePath);
        hashList(mazePath);
        removeRandomWalls(mazePath);
        return mazePath;
    }

    private static void removeRandomWalls(String[][] mazePath) {
        int numberToRemove = (mazePath.length-1) / 8;
        int center = (mazePath.length - 1) / 2;
        int i, j;
        while (numberToRemove > 0) {
            i = (int) (center - center / 2 + (Math.random() * center));
            j = (int) (center - center / 2 + (Math.random() * center));
            if (mazePath[i][j].equals("|") || mazePath[i][j].equals("-")) {
                mazePath[i][j] = "#";
                numberToRemove--;
            }
        }
